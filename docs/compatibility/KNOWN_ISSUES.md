...
The new "Preload Shader Cache" toggle makes that step blocking instead: the previously-stored cache finishes warming up (using all available cores, same as before) *before* the title starts, eliminating that pop-in on a return playthrough. First-time launches with nothing cached yet aren't affected either way. **Trade-off, and why it's off by default:** turning this on trades a longer pause between clicking a game and it starting for a smoother playthrough once it does. Verified to build and link correctly; **not yet confirmed on real Xbox hardware** - specifically whether that pause reads as "loading" or as "frozen" needs an on-device check with a title that already has a warm cache (play it once, exit, relaunch it, toggle the setting).

**Investigated and ruled out: an animated on-screen progress bar during that pause.** `PipelineCache` now tracks warm-up progress (`GetShaderStoragePreloadTotal/Done`, threaded up through `CommandProcessor::GetShaderStoragePreloadProgress`) so the data exists, but there's no safe way to *draw* it live: `Emulator::CompleteLaunch` (where the blocking wait happens) unconditionally re-dispatches itself onto the UI thread if called from anywhere else - it touches the window icon and game config UI callbacks - so moving title launch to a background thread doesn't free up the frontend to animate, it just adds a hop before the same freeze happens on the same thread anyway. Making this actually animated would mean changing that dispatch behavior in Xenia's own launch path, which isn't something to do blind without a real title to test the launch sequence against afterward. Left as a real pause, not a fake spinner over a frozen thread.
