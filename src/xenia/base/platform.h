...
// Hardcoded rather than derived from WINAPI_FAMILY_PARTITION, despite that
// looking like the "correct" per-translation-unit signal for an actual UWP
// app-container build: xenia-base/xenia-kernel/xenia-ui/etc. are compiled
// exactly once, by a single ordinary desktop CMake configuration (see
// xenia-canary-uwp/CMakeLists.txt), and that same object code is linked as
// prebuilt .lib output directly into the real Xbox/UWP binary via the
// xenia-canary-uwp.vcxproj's AdditionalDependencies - there is no separate
// UWP-flavored compile of those shared libraries for WINAPI_FAMILY_PARTITION
// to correctly reflect. Deriving this from WINAPI_FAMILY_PARTITION would
// make those libraries always take the desktop branch (WINAPI_PARTITION_
// DESKTOP is available to CMake's own build), silently breaking real
// hardware: GetUserFolder()'s storage-folder resolution in filesystem_win.cc,
// xinput_input_driver.cc's UWPWindow casts, and the on-screen keyboard
// invocations in signin_ui.cc/create_profile_ui.cc would all resolve to
// their desktop answer even when linked into the Xbox app.
//
// The desktop xenia-app CMake target still needs XE_PLATFORM_WINRT-gated
// code (Vulkan/Discord/SDL-input registration in xenia_main.cc, the missing
// UWP:: definitions) to behave like a normal desktop build. That's handled
// per-translation-unit instead, only for files the vcxproj doesn't also
// compile itself: xenia_main.cc locally corrects XE_PLATFORM_WINRT for just
// its own translation unit when XE_APP_DESKTOP_BUILD is defined (see the
// comment there), and xenia/app/uwp_stubs_win.cc supplies the missing UWP::
// definitions - neither approach touches this hardcoded value or the shared
// libraries' own compile.
