...
bool ContentPackageContainer::LoadPackage(
  ...
  // Memory-mapping can fail for paths outside the app's normal sandbox
  // (e.g. content picked via the file picker). Not applicable to kReadWrite:
  // BufferedMappedMemory never flushes back to the file.
  if (!package_ && open_mode == MappedMemory::Mode::kRead) {
    auto host_file = xe::filesystem::OpenFile(package_path, "rb");
    if (host_file) {
      const size_t host_size =
          static_cast<size_t>(std::filesystem::file_size(package_path));
      ...
      if (bytes_read == buffered_data.size()) {
        XELOGW("Falling back to buffered read for {} on UWP.",
               package_path.string());
        package_ = std::make_unique<xe::BufferedMappedMemory>(
            std::move(buffered_data));
      }
    }
  }
#endif
